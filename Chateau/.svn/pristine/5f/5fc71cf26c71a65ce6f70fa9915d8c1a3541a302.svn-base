		...
		var DarkTR = document.getElementsByTagName('td');
			for(i=0;i<DarkTR.length;i++){
				if(attentionArray.indexOf(DarkTR[i].parentNode.parentNode.parentNode.id)!=-1){
					DarkTR[i].style.background = '#dfefec url(../css/default/img/document_hot.gif) no-repeat 3px 4px';
					...
					DarkTR[i].innerHTML = TEXT + '<img style=\"margin-left:10px;\" src=\"../skin/default/img/notice.gif\">'; /*自动为已加入关注的内容后增加对应文字或图示*/
					//alert('YES');					
					}
			}
			...
			var imptt = document.getElementById('Myimportant');
				if((attentionArray.push('${empAtt}')-1) == 0){
					...
					imptt.innerHTML = '<a href=\"empAttentionListAction.do\" title=\"点击查看详细列表\" >' + '<img style=\"position:relative;top:-1px;margin-right:8px;\" src=\"../skin/default/img/notice.gif\">' + "我已有" + "<span>" + "<font color=#990000>" + (attentionArray.push('${empAtt}')-2) +  "</font>" + "</span>" + "条关注的工作" + "</a>";
					}
	...
	*/
	...
	function ShowMouseOver(obj){
		...
		var DarkTD = obj.parentNode;
			if(obj.className == "CTM_row_dark_focus"){
			obj.style.background = "#e5f0ee url(../css/default/img/document_add.gif) no-repeat 3px 4px";
	...
	function ShowMouseOut(obj){
		...
		var DarkTD = obj.parentNode;
			if(obj.className == "CTM_row_dark_focus"){
			obj.style.background = "#eef4f3 url(../css/default/img/document.gif) no-repeat 3px 4px";

...
function sendMessage() {
    ...
    if (name == '' || name == '全名*' || email == '' || email == '邮箱地址*' || subject == '' || subject == '主题*' || msg == '' || msg == '你的问题*') {
        $("div#msgs").html('<p class="warning">你必须全部填完所有内容!</p>');
    ...
    if (!checkEmail(email)) {
        $("div#msgs").html('<p class="warning">请输入正确的邮箱格式</p>');
        return false;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success == 0) {
                ...
                $("div#msgs").html('<p class="error">成功提交问题!</p>' + errors);
            }
            else if (data.success == 1) {

                $("div#msgs").html('<p class="error">成功提交问题!</p>');
            }
